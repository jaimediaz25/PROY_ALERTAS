...
<head>
    ...
    <style>
        ...
        .navbar-brand {
            ...
            font-weight: bold;
            text-shadow: none; /* Sin neón en el texto "Alertas" */
        }
    ...
    <div class="container">
        ...
        <div class="d-flex justify-content-between">
            <form method="GET" action="{{ route('alerts.index') }}" class="d-flex align-items-center">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar alerta..." class="form-control mb-3" style="width: 250px;">
                <button type="submit" class="btn btn-primary btn-sm mb-3">Buscar</button>
            </form>

            <div>
                <a href="{{ route('alerts.create') }}" class="btn btn-primary btn-sm mb-3">Nueva Alerta</a>
                <a href="{{ route('alerts.grafica') }}" class="btn btn-success btn-sm mb-3">Gráfica</a>
                <a href="{{ route('alerts.exportPdf', request()->query()) }}" target="_blank" class="btn btn-info btn-sm mb-3">Descargar PDF</a>
                <a href="{{ route('alerts.exportExcel', request()->query()) }}" class="btn btn-info btn-sm mb-3">Descargar Excel</a>
                <form action="{{ route('alerts.importExcel') }}" method="POST" enctype="multipart/form-data" class="import-form">
                    @csrf
            ...
            <tbody>
                @foreach($alerts as $alert)
                    <tr>
                        ...
                        <td>{{ $alert['mensaje'] }}</td>
                        <td>{{ $alert['atendida'] }}</td>
                        <td>{{ $alert['generado_en'] }}</td>
                        <td>
                            <a href="{{ route('alerts.edit', $alert['_id']) }}" class="btn btn-warning btn-sm">Editar</a>
                            <form action="{{ route('alerts.destroy', $alert['_id']) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
